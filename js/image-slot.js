...
(() => {
  ...
  function load() {
    ...
    // Sidecar persistence is an authoring-only feature (writes go through
    // window.omelette). In production there is no host and no sidecar file,
    // so skip the fetch to avoid a cosmetic 404 — slots render from `src`.
